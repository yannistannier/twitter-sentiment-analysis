...
if __name__ == "__main__":
    ...
    sc._jsc.hadoopConfiguration().set("fs.s3a.access.key", "xxxxxxxx")
    sc._jsc.hadoopConfiguration().set("fs.s3a.secret.key", "xxxxxxxxx")


    myRDD = sc.textFile("s3a://bucket/"+sys.argv[1])
    ...
    analyzed = myRDD.map(lambda line: json.loads(line))\
        ...
        .map(lambda d: [{ "data" : d, "hashtag": h['text']} for h in d["entities"]["hashtags"] ]   )\
        .flatMap(lambda x: x)\
        .map(tw.formatter)

    analyzed.saveAsTextFile("s3a://bucket/"+sys.argv[2]+"/")
