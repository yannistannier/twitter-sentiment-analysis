...
def test(spark):
    ...
    df = df.groupBy("retweet").agg(array(*[somme(col("emotion")[i]) for i in range(n)]).alias("sum_emotion"),array(*[somme(col("sentiment")[i]) for i in range(m)]).alias("sum_sentiment"))
