...
class User extends Component {
  ...
  componentDidMount() {
      ...
      if(this.state.results.length == 0) {
          ...
          axios.get('https://xxxxxxx.execute-api.eu-west-1.amazonaws.com/twitter/best-user?id=0')
                  ...
                  <Col xs="12" sm="6" md="6">
                    ...
                    <Table hover responsive className="table-outline mb-0 d-none d-sm-table">
                      ...
                      {this.state.results.map((item, index) => (
                        <tr key={index}>
                          <td>
                            <div style={{color:'#1DA1F2', fontWeight:'bold', fontSize:14}}><a href={"#/archive/user/" + item.username}>{item.username}</a></div>
                          </td>
                          <td >
                            {item.total}
                          </td>
